...
import type { Competition, ArrangementVersion, GridPoint } from './types'
...
export function CompetitionLevels({ competition, controller, username }: {
  ...
  const [located, setLocated] = useState<{registrationId:string;points:GridPoint[];message:string} | null>(null)
  useEffect(()=>setPrintJob(null),[id,selectedVersion])
  useEffect(()=>{setLocated(null);setSelectedVersion(null);setHistorical(null)},[id])
  ...
  useLayoutEffect(()=>{
    const content=contentRef.current,sheet=sheetRef.current,table=sheet?.querySelector('.arrangement-table-scroll')
    if(!content||!sheet||!table)return
    const measure=()=>{
      const box=table.getBoundingClientRect(),top=box.top-sheet.getBoundingClientRect().top
      content.style.setProperty('--arrangement-table-height',`${box.height}px`)
      content.style.setProperty('--arrangement-table-top',`${top}px`)
    }
    measure()
    const observer=new ResizeObserver(measure);observer.observe(table);observer.observe(sheet)
    return()=>observer.disconnect()
  },[id,selectedVersion,layout,historyOpen])
  useLayoutEffect(()=>{
    const previous=historyView.current
    if(historyOpen && versionList.current && (previous.id!==id||!previous.opened||(newestSequence>previous.latest&&(!selectedVersion||previous.latest<0)))) {
  ...
  function locateChange(change: (typeof changes)[number]) {
    ...
    const previousPoint=prior?{row_id:prior.row_id,column_id:prior.column_id}:null
    const currentPoint=current?{row_id:current.row_id,column_id:current.column_id}:null
    const requested=change.kind==='added'?[currentPoint]:change.kind==='removed'?[previousPoint]:[previousPoint,currentPoint]
    const points=requested.filter((point):point is GridPoint=>available(point))
    const unique=[...new Map(points.map(point=>[`${point.row_id}/${point.column_id}`,point])).values()]
    let message=''
    if(change.kind!=='added'&&!oldLayout)message='前版未記錄格位，無法定位來源。'
    else if(change.kind!=='added'&&!previousPoint)message='比較前版沒有這位選手的格位紀錄。'
    else if(previousPoint&&!available(previousPoint))message='來源格位已不在目前布局。'
    if(change.kind!=='removed'&&!currentPoint)message=message?`${message} 目標格位也未記錄。`:'目前版本沒有可定位的目標格位。'
    else if(currentPoint&&!available(currentPoint))message=message?`${message} 目標格位已不在目前布局。`:'目標格位已不在目前布局。'
    if(!unique.length&&!message)message='此差異沒有目前布局中可定位的格位。'
    setLocated({registrationId:change.row.registration_id,points:unique,message})
  }
  function chooseVersion(versionId:string|null) { setLocated(null);setSelectedVersion(versionId) }
  const pendingStatus=operation?.status??big?.status
  const statusText=pendingStatus==='saving'?'正在儲存…':workspace?.loading?'讀取中…':pendingStatus==='unknown'?'操作結果尚未確認，請使用下方恢復按鈕。':pendingStatus==='refresh'?'已取得回執，等待讀回確認。':pendingStatus==='rejected'?'操作遭拒絕，請重新讀取並核對。':workspace?.error?'讀取失敗，這場暫停修改。':selectedVersion?'歷史版本唯讀':readonly?'此場次為唯讀':workspace?.notice||(state?.latest?`上次保存：${state.latest.label} · ${time(state.latest.created_at)}`:'')
  const canExport=!busy&&!workspace?.loading&&!!state&&(!selectedVersion||!!historical)&&!exporting
  async function exportExcel(){
    if(!canExport)return
    const snapshot=structuredClone({rows,layout,title:competition.name,version:selectedVersion?`歷史：${historical!.current.label}`:'目前安排'})
    setExporting(true);setExportError('')
    try{
      await exportArrangementExcel(snapshot)
    }
    catch{setExportError('Excel 匯出失敗，請再試一次。')}
  ...
  return <section className="arrangement-workspace" aria-label="當次比賽級數安排">
    ...
    <div className="arrangement-toolbar"><div><h2>{competition.name}</h2><small>{selectedVersion ? `回看：${historical?.current.label ?? '讀取中'}` : '目前安排'}</small></div><div className="arrangement-tools">
      ...
      {selectedVersion ? <button onClick={() => { chooseVersion(null); setHistorical(null) }}>回目前安排</button> : <button disabled={!canSave} onClick={() => { setLabel(workspace?.draft?.label ?? `版本 ${(state?.latest?.sequence ?? 0) + 1}`); setEditor(workspace?.draft?.editor_label ?? username); setNote(workspace?.draft?.note ?? ''); setSaveOpen(true) }}>保存完整安排</button>}
    ...
    <div ref={contentRef} className={`arrangement-content ${historyOpen ? 'with-history' : ''}`}>
      ...
      {historyOpen && <>
        ...
        <div className="arrangement-diff-scroll">{!oldLayout && selectedVersion && <p>前版未記錄位置，無法比較位置差異。</p>}{!changes.length && !structural && <p>沒有可比較的安排變更</p>}
        <ul className="arrangement-changes">{changes.map(change => <li className={`net-${change.kind}`} key={change.row.registration_id}><button type="button" className="arrangement-change-card" aria-pressed={located?.registrationId===change.row.registration_id} onClick={()=>locateChange(change)}><strong>{change.row.member_name}</strong>{change.row.distinguishing_note && <small>{change.row.distinguishing_note}</small>}<span>{changeText(change)}</span></button>{located?.registrationId===change.row.registration_id&&located.message&&<small className="arrangement-change-location" role="status">{located.message}</small>}</li>)}</ul></div>
        </aside>
      <aside className="arrangement-history" aria-label="版本紀錄"><div className="arrangement-panel-heading"><h3>版本紀錄</h3><button className="secondary" aria-pressed={!selectedVersion} onClick={() => chooseVersion(null)}>目前安排</button></div>
        <div className="arrangement-version-list" ref={versionList} aria-label="保存版本，由舊到新">{orderedVersions.map(version => <button className={`version-item secondary ${selectedVersion === version.id ? 'selected' : ''}`} key={version.id} data-version-sequence={version.sequence} onClick={() => chooseVersion(version.id)}><strong>{version.label}{version.sequence===newestSequence&&<span className="version-latest">最新</span>}</strong><small>{time(version.created_at)}</small><small>顯示編輯者：{version.editor_label}</small></button>)}</div>
