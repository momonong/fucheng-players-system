...
export function AdminHeader({ section, username, onLogout }: { section: 'members' | 'competitions' | 'announcements'; username: string; onLogout: () => void }) {
  const titles = { members: '會員管理', competitions: '比賽管理', announcements: '公告管理' }
  ...
  return <header className="admin-header no-print">
    ...
    <div className="admin-header-controls"><nav className="admin-nav" aria-label="管理功能">
      {([['members', '/admin'], ['competitions', '/admin/competitions'], ['announcements', '/admin/announcements']] as const).map(([key, href]) =>
