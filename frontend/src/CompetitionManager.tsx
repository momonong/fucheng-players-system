...
export function CompetitionManager({ username, onLogout }: { username: string; onLogout: () => void }) {
  ...
  async function loadList(selectId?: string) {
    try {
      const result = await api.competitions()
      setItems(result)
      const next = selectId ?? selectedId ?? result[0]?.id ?? null
      setSelectedId(next)
      if (next) setDetail(await api.competition(next))
    } catch (error) {
  ...
  async function loadDetail(id: string) {
    ...
    try { setDetail(await api.competition(id)) }
  ...
  return <>
    <header className="admin-header no-print">
      ...
      <div className="header-actions"><span>{username}</span><a href="/admin">會員管理</a><a href="/">公開名單</a><button className="secondary" onClick={signOut}>登出</button></div>
    </header>
    ...
    <main className="competition-layout">
      <aside className="panel competition-list no-print">
        <div className="section-title"><div><h2>比賽</h2><p>共 {items.length} 場</p></div><button onClick={() => { setCreating(true); setSelectedId(null); setDetail(null) }}>建立比賽</button></div>
        <div className="competition-items">{items.map(item => <button key={item.id} className={`competition-item ${selectedId === item.id ? 'selected' : ''}`} onClick={() => loadDetail(item.id)}>
          <strong>{item.name}</strong><span>{item.competition_date}・{statusText[item.status]}</span><small>正取 {item.summary.confirmed}/{item.capacity}・候補 {item.summary.waitlisted}</small>
        </button>)}</div>
      </aside>
...
function CompetitionEditor({ competition, onSaved }: { competition?: Competition; onSaved: (value: Competition) => void }) {
  ...
  const readonly = competition?.status === 'ended' || competition?.status === 'cancelled'
...
function CompetitionRoster({ detail, reload, notify }: {
  ...
  const readonly = competition.status === 'ended' || competition.status === 'cancelled' || competition.status === 'draft'
  useEffect(() => {
    let active = true
