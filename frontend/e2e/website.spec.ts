...
test('首頁公告發布下架、免登入分級與比賽導覽', async ({ page, browser }, testInfo) => {
  ...
  await page.getByRole('combobox', { name: '文字樣式' }).selectOption('h1')
  ...
  await expect(visitor.locator('.news-card').filter({ hasText: title }).locator('img')).toBeVisible()
  ...
  await expect(page.getByLabel('公告內容')).toContainText('合成衝突時保留內容')
  ...
  await expect(page.locator('.announcement-workspace li')).toHaveCount(4)
  await context.close()
})
